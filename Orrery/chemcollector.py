...
def fetch_chembl(DB_NAME = 'universe', COLLECTION = 'chembl', limit = 1000, rdkit_obj = True, max_pages = None):
    ...
    collection = chembl_db.DB[COLLECTION]
    ...
    while url:
        ...
        print(type(batch))
        input('aosidnoaisdn')
        # Prepare & execute bulk operations
        operations = [UpdateOne({"smiles": doc["smiles"]}, {"$set": doc}, upsert=True) for doc in batch]
        try:
...
def fetch_pdbbind(email='samuelmar@gmail.com', pw='J@7iD6i7Lhnugqj', db='universe', collection_name='pdbbind'):
    ...
    if insert:
        ...
        ghost_indexes = []
        print('Sending additional data to local mongo db...')
        for document in tqdm(documents):
            ...
            #if not index_found: # uncomment if ghost indices are needed
                #ghost_indexes.append(document['index'])  # save ghost indices for potential future use
